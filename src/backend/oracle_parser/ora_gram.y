 ...
 *	  because the entire string is parsed by ora_gram.y before the SET gets
 ...
 * the set of keywords.  PL/SQL depends on this so that it can share the
 * same lexer.  If you add/change tokens here, fix PL/SQL to match!
 *
 ...
 */
merge_when_clause:
		merge_when_tgt_matched THEN merge_update
			{
				$3->matchKind = $1;
				$3->commandType = CMD_UPDATE;

				$$ = (Node *) $3;
			}
		| merge_when_tgt_not_matched THEN merge_insert where_clause
			{
				$3->matchKind = $1;
				$3->commandType = CMD_INSERT;
				$3->condition = $4;
					...
					| '(' expr_list ',' '*' ')'
						{
							ereport(ERROR,
									(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
									 errmsg("NUMERIC scale specifier is out of range"),
									 parser_errposition(@4)));
						}
					| '(' '*' ',' '*' ')'
						{
...
Numeric:	INT_P
			...
			| BINARY_FLOAT
				{
					$$ = OracleSystemTypeName("binary_float");
					$$->location = @1;
				}
			| BINARY_DOUBLE
				{
...
case_expr:	CASE case_arg when_clause_list case_default END_P
			...
			| DECODE '(' expr_list ')'
				{
					List *pars_list = $3;
					int len1 = list_length(pars_list);

					/* if len < 3, return binary string function with the same name decode */
					if(len1 < 3)
					{
						FuncCall *n =NULL;
						List *func_name =NIL;

						func_name = list_make1(makeString(pstrdup($1)));
						n = makeFuncCall(func_name, pars_list, COERCE_EXPLICIT_CALL, @1);
						$$ = (Node *) n;
					}
					else
					{
						...
						c->casetype = InvalidOid; /* not analyzed yet */
						c->arg = (Expr *) lfirst(list_head(pars_list));

						cell = lnext(pars_list, list_head(pars_list));
						for (; i < len2; i++)
						{
							CaseWhen *w = makeNode(CaseWhen);

							w->expr = (Expr *) lfirst(cell);
							w->orig_expr = copyObject(w->expr);
							cell = lnext(pars_list, cell);
							w->result = (Expr *) lfirst(cell);
							cell = lnext(pars_list, cell);
							w->location = @3;
							w_expr = lappend(w_expr, w);
						}
						c->args = w_expr;

						if (len1 % 2 == 0)
							c->defresult = (Expr *) lfirst(cell);
						else
							c->defresult = NULL;
						c->location = @1;
						c->is_decode = true;
			...
			| NVL  '(' a_expr ',' a_expr ')'
				{
					...
					a_expr = list_make1($3);
					expr_list = lappend(a_expr,$5);
					n = makeNode(CoalesceExpr);
					n->args = expr_list;
					n->location = @1;
					res->val = (Node *) n;
					res->location = @1;
			...
			| NVL2 '(' a_expr ',' a_expr ',' a_expr ')'
				{
					List *args = NIL;
					NullTest *n_expr = NULL;
					CaseExpr *c = NULL;
					CaseWhen *w = NULL;
					...
					{
						n_expr = makeNode(NullTest);
						n_expr->arg = (Expr *) $3;
						n_expr->nulltesttype = IS_NULL;
						n_expr->location = @3;
 ...
 */
void
ora_parser_init(ora_base_yy_extra_type *yyext)
{
